...
public class RotationMatrix {
    ...
    public static Point3 rotPoint(Point3 ax1, Point3 ax2, Point3 p, double deg){
    	double[] pts = rotPointFromFormula(ax1.getX(), ax1.getY(), ax1.getZ(), ax2.getX(), ax2.getY(), ax2.getZ(), p.getX(), p.getY(), p.getZ(), Math.toRadians(deg));
    	p.setPosition((float)pts[0], (float)pts[1], (float)pts[2]);
    	return p;
    }
    
    public static Point3 rotPoint(Line3 ax, Point3 p, double deg) {
        Point3[] ps = ax.getTwoPoints();
        return rotPoint(ps[0], ps[1], p, deg);
    }
    
    public static Vector3 rotVector(Vector3 v1, Vector3 v2, double deg) {
         double[] pts = rotPointFromFormula(0, 0, 0, v1.getX(), v1.getY(), v1.getZ(), v2.getX(), v2.getY(), v2.getZ(), Math.toRadians(deg));
         v2.setPosition((float)pts[0], (float)pts[1], (float)pts[2]);
         return v2;
    }
